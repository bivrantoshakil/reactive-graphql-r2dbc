...
import jp.co.anyx.util.toPaymentResponse
...
class SalesServiceImpl(
    ...
    @Cacheable("sales")
    override fun getHourlySalesStatement(timeRange: TimeRangeRequest): Mono<SalesResponse> {
        val salesList = paymentRepository.findSalesStatementGroupedByInterval(
            startDateTime = timeRange.startDateTime,
            endDateTime = timeRange.endDateTime,
            interval = SalesStatementInterval.HOURLY.toString()
        )
            .retry(retryCount)
            .collectList().map {
    ...
    override fun createPayment(paymentRequest: PaymentRequest): Mono<PaymentResponse> {
        val paymentConfigMono = paymentRequest.getValidPaymentConfig()

        val savedPayment = paymentConfigMono.flatMap { paymentConfig ->
